...
Service.prototype.healthcheck = function(name, fn) {
  this.healthchecks.push({ name: name, fn: fn });
}
...
Service.prototype.registerService = function(done) {
  ...
  var doHealthcheck = function(cb) {

    if(!self.healthchecks.length)
      return cb(true);

    async.each(self.healthchecks, function(check, done) {
      check.fn(done);
    }, function(err) {
      if(err) {
        cb(false);
  ...
  var doRegister = function(cb) {
    ...
    doHealthcheck(function(healthy) {
